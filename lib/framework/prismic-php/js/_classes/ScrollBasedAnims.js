...
export class ScrollBasedAnims {
			...
			for (let i = 0; i < length; i++) {
				...
				if (data.splitPrepped) {
					if ((this.data.scrollY > (data.bottom + 50)) && data.played && !data.splitReset) {
						...
						data.splits[0].revert();
						data.splits[1].revert();
						data.splits[2].revert();
					}
				}
				...
				if (this.thisPagesTLs[i] === "split-copy" && !data.splitPrepped) {
					if ((this.data.scrollY + data.offset + (this.data.height * 1.5)) > data.top) {
						...
						let splitEl = this.dom.scrollBasedElems[i]

						let splitLinesInstance = new SplitText(splitEl, {type: "lines"}),
							splitLinesWordsInstance = new SplitText(splitLinesInstance.lines, {type: "words"}),
							splitLinesCharsInstance = new SplitText(splitLinesWordsInstance.words, {type: "chars"});

						data.splits.push(splitLinesInstance)
						data.splits.push(splitLinesWordsInstance)
						data.splits.push(splitLinesCharsInstance)
						let splitLinesChars = splitLinesCharsInstance.chars;

						let entranceTL = new gsap.timeline({ paused: true });

						entranceTL.set(this.dom.scrollBasedElems[i], { opacity: 1 });

						entranceTL.fromTo(splitLinesChars, { x: -6, y: 8, scale: 1.1, blur: 4, opacity: 0 }, { duration: 0.5, scale: 1, opacity: 1, x: 0, y: 0, blur: 0, ease: "sine.inOut", stagger: 0.06, force3D: true })
	...
	getBounding() {
		...
		this.data.max = Math.floor((this.dom.el.querySelector(".view-home").getBoundingClientRect().height - this.data.height) + this.data.scrollY);
	}
